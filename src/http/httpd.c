...
zbool_t zhttpd_request_read_all(zhttpd_t *httpd)
{
...
zbool_t zhttpd_response_200(zhttpd_t *httpd, const char *data, int size)
{
    if (size < 0)
    {
        size = strlen(data);
    ...
    zhttpd_response_flush(httpd);
    return (1 - zhttp_is_exception(httpd));
}

zbool_t zhttpd_response_301(zhttpd_t *httpd, const char *url)
{
...
zbool_t zhttpd_response_302(zhttpd_t *httpd, const char *url)
{
...
zbool_t zhttpd_response_304(zhttpd_t *httpd, const char *etag)
{
...
zbool_t zhttpd_response_404(zhttpd_t *httpd)
{
...
zbool_t zhttpd_response_500(zhttpd_t *httpd)
{
...
zbool_t zhttpd_response_501(zhttpd_t *httpd)
{
...
{
    return zstream_write(httpd->fp, data, len);
}

int zhttpd_response_puts(zhttpd_t *httpd, const char *data)
{
    return zstream_write(httpd->fp, data, strlen(data));
}

int zhttpd_response_append(zhttpd_t *httpd, const zbuf_t *bf)
{
    return zstream_write(httpd->fp, zbuf_data(bf), zbuf_len(bf));
}
