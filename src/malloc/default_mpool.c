...
static int zmem_default_pool_set_cmp(zrbtree_node_t * rn1,
        zrbtree_node_t * rn2)
{
...
static zmem_default_pool_set_t *zmem_default_pool_set_create(
        ...
        zmem_default_pool_setgroup_t * setgroup)
{
    ...
    data = (char *)zmalloc(sizeof(zmem_default_pool_set_t) +
            setgroup->element_size * setgroup->element_count_per_set);
...
void *zmem_default_pool_malloc(zmpool_t *mp, int len)
{
    ...
    if (setgroup->set_used.head == 0)
    {
        ...
        {
            set = ZCONTAINER_OF(setgroup->set_unused.head,
                    zmem_default_pool_set_t, linknode);
...
static inline zmem_default_pool_set_t *find_set(zmem_default_pool_t *worker, void *ptr)
{
    zmem_default_pool_setgroup_t *setgroup_list, *setgroup;
    ...
    if (((char *)ptr - (char *)(set->element_list)) >
            ((unsigned int)(setgroup->element_count_per_set) *
             (unsigned int)(setgroup->element_size)))
    {
        return 0;
    }
...
static inline void free_by_set(zmem_default_pool_t *worker,
        zmem_default_pool_set_t *set, void *ptr)
{
...
void * zmem_default_pool_realloc(zmpool_t *mp, void *ptr, int len)
{
    zmem_default_pool_t * worker = (zmem_default_pool_t *)mp->worker;
    zmem_default_pool_set_t *set;
    zmem_default_pool_setgroup_t *setgroup_list, *setgroup;
...
void zmem_default_pool_free(zmpool_t *mp, void *ptr)
{
    zmem_default_pool_t * worker = (zmem_default_pool_t *)mp->worker;
    zmem_default_pool_set_t *set;
...
zmpool_t *zmpool_create_default_pool(int *register_list)
{
    ...
    for(; register_one && *register_one; register_one++)
    {
        ...
        {
            zfatal("zmem_default_pool_create: element_size must > %d ",
                    (int)(sizeof(unsigned short int)));
        }
        register_count++;
    }

    mp = (zmpool_t *) zcalloc(1, sizeof(zmpool_t) +
            sizeof(zmem_default_pool_t) +
            sizeof(zmem_default_pool_setgroup_t)*register_count);
