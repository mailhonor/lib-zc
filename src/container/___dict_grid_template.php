<?php
function ___show($type, $subtype, $fn)
{
    ob_start();
?>/*
 * ================================
...
<?
    ...
    {
?>
zgrid_node_t *zgrid_add(zgrid_t * mp, <?if($subtype=='')echo 'char *';else echo 'long ';?>key, void *value, char **old_value)
<?
    }
    else
    {
?>
zgrid_node_t *zgrid_add(zgrid_t * mp, <?if($subtype=='')echo 'char *';else echo 'long ';?>key, char *value)
<?
    }
?>
{
...
?>
    ...
    else
    {
        ...
        mp_np->key = <?if($subtype=="") echo 'zmpool_strdup(mp->mpool, key)'; else echo 'key';?>;
        mp_np->value = <?if($type=="grid") echo 'value';else echo 'zmpool_strdup(mp->mpool, value)';?>;
...
zgrid_node_t *zgrid_lookup(zgrid_t * mp, <?if($subtype=='')echo 'char *';else echo 'long ';?>key, char **value)
{
    zgrid_node_t mp_n, *mp_np;
    zrbtree_node_t *rb_np;
...
zgrid_node_t *zgrid_lookup_near_prev(zgrid_t * mp, <?if($subtype=='')echo 'char *';else echo 'long '?>key, char **value)
{
    zgrid_node_t mp_n, *mp_np;
    zrbtree_node_t *rb_np;
...
zgrid_node_t *zgrid_lookup_near_next(zgrid_t * mp, <?if($subtype=='')echo 'char *';else echo 'long ';?>key, char **value)
{
    zgrid_node_t mp_n, *mp_np;
    zrbtree_node_t *rb_np;
...
<?
    ...
    {
?>
void zgrid_delete(zgrid_t * mp, <?if($subtype=='')echo 'char *';else echo 'long ';?>key, char **old_value)
{
    zgrid_node_t *n;
...
<?
    ...
    {
?>
void zgrid_delete(zgrid_t * mp, <?if($subtype=='')echo 'char *';else echo 'long ';?>key)
{
    zgrid_node_t *n;

    n = zgrid_lookup(mp, key, 0);
<?
    }
?>
    if (!n)
...
int zgrid_keys(zgrid_t * mp, <?if($subtype=='')echo 'char *';else echo 'long ';?>*key_list, int size)
{
...
<?
    ...
    $type = "z".$subtype.$type;
    $content = str_replace("zgrid", $type, $content);
...
___show("grid", "", "grid.c");
___show("grid", "i", "igrid.c");
___show("dict", "", "dict.c");
___show("dict", "i", "idict.c");
