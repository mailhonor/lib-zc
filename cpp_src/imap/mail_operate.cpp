...
int imap_client::cmd_append_prepare_protocol(append_session &append)
{
    ...
    {
        char timebuf[128 + 1];
        std::tm *now_tm = localtime(append.time_);
        std::string r;
    ...
    {
        zcc_imap_client_debug("ERROR read");
        return -1;
    }
    zcc_imap_client_debug_protocol_read(linebuf);
    if (linebuf[0] != '+')
    {
        zcc_imap_client_debug("ERROR want '+ ...' instead of (%s)", linebuf.c_str());
        return 0;
    }
    return 1;
}
...
int imap_client::append_file(uidplus_result &result, append_session &append, const char *filename)
{
    ...
    if (!fp)
    {
        zcc_imap_client_debug("ERROR open %s", filename);
        goto over;
    }
    ...
    while (left > 0)
    {
        ...
        if (len < 1)
        {
            zcc_imap_client_debug("ERROR read %s", filename);
            need_close_connection_ = true;
            goto over;
        }
        if (fp_->write(buf, (int)len) < 0)
        {
            zcc_imap_client_debug("ERROR write");
            need_close_connection_ = true;
            goto over;
        }
        left -= len;
