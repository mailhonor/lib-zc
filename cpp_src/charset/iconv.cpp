...
extern "C"
{
    typedef void *my_iconv_t;
    typedef my_iconv_t (*my_iconv_open_fn_t)(const char *__tocode, const char *__fromcode);
    typedef int (*my_iconv_close_fn_t)(my_iconv_t __cd);
    typedef size_t (*my_iconv_fn_t)(my_iconv_t __cd, char **__restrict __inbuf, size_t *__restrict __inbytesleft, char **__restrict __outbuf, size_t *__restrict __outbytesleft);
    static void *my_iconv_db_handler = nullptr;
    static my_iconv_open_fn_t my_iconv_open = nullptr;
    static my_iconv_close_fn_t my_iconv_close = nullptr;
    static my_iconv_fn_t my_iconv = nullptr;
}
#else
#include <iconv.h>
 ...
 */
std::string iconv_convert(const char *from_charset, const char *str, int64_t str_len, const char *to_charset, int64_t *invalid_bytes)
{
    // 记录转换过程中无效字节的数量
    int err_bytes = 0;
    ...
    // 打开一个 iconv 转换描述符
    my_iconv_t ic = my_iconv_open(to_charset, from_charset);
    // 如果打开失败，则返回空字符串
    if (ic == (my_iconv_t)-1)
    {
        return "";
    ...
    {
        // 重置 iconv 转换状态
        my_iconv(ic, NULL, NULL, NULL, NULL);
        // 临时保存输出缓冲区的大小和指针
        ...
        // 执行字符集转换
        size_t ret = my_iconv(ic, &in_str, &in_len, &out_str_tmp, &out_len_tmp);
        // 计算转换后输出字符串的长度
        ...
        if (ret != (size_t)-1)
        {
            ...
            // 处理剩余的输出
            my_iconv(ic, NULL, NULL, &out_str_tmp, &out_len_tmp);
            // 计算剩余输出的长度
