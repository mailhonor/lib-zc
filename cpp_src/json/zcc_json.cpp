...
static inline int _empty(const void *ptr) { return ((!ptr) || (!(*(const char *)(ptr)))); }
...
int64_t &json::get_long_value(int64_t def)
{
    int64_t v = def;
...
bool json::object_delete(const std::string &key, json **old)
{
    json *tmpj = 0;
    used_for_object();
    std::map<std::string, json *> &m = *(val_.m);
...
bool json::get_value_by_path(const char *name, int64_t *value)
{
    json *j = get_by_path(name);
    if (!j)
    {
...
bool json::load_from_file(const char *pathname)
{
    std::string con;
    int ch;
...
static inline const char *___ignore_blank(const char *ps, const char *str_end)
{
    while (ps < str_end)
    {
        if ((*ps != ' ') && (*ps != '\t') && (*ps != '\r') && (*ps != '\n'))
...
static inline int ___ncr_decode(int ins, char *wchar)
{
    if (ins < 128)
    {
        *wchar = ins;
...
bool json::unserialize(const char *jstr, int jlen)
{
    ...
    while (ps < str_end)
    {
        ...
        if ((!std::strcmp(tmpkey.c_str(), "null")) || (!std::strcmp(tmpkey.c_str(), "undefined")))
        {
            current_json->reset();
            continue;
        }
        if (!std::strcmp(tmpkey.c_str(), "true"))
        {
            current_json->get_bool_value() = true;
            continue;
        }
        if (!std::strcmp(tmpkey.c_str(), "false"))
        {
...
static inline void ___serialize_string2(std::string &result, const std::string &str)
{
...
int64_t json::object_get_long_value(const std::string &key, int64_t def)
{
    json *js = object_get(key);
    if (!js)
    {
