...
static void *zmem_grow_pool_realloc(zmpool_t * mp, const void *ptr, int len)
{
    zmem_grow_pool_t *worker = (zmem_grow_pool_t *) (mp->worker);
...
void zmem_grow_pool_free(zmpool_t * mp, const void *ptr)
{
    zmem_grow_pool_t *worker = (zmem_grow_pool_t *) (mp->worker);
    zmem_grow_pool_one_t *one;
    int i;
...
void zmem_grow_pool_free_pool(zmpool_t * mp)
{
    ...
    if (worker->direct_list) {
        ...
        ZARRAY_WALK_END;
        zarray_free(worker->direct_list);
    }
