 ...
 */
static void *body(zmd5_t * ctx, const void *data, size_t size)
{
    unsigned char *ptr;
...
void zmd5_final(zmd5_t * ctx, void *result)
{
    /* @UNSAFE */
    unsigned long used, free;
...
char *zmd5(const void *data, int size, void *result)
{
    ...
    zmd5_init(&ctx);
    zmd5_update(&ctx, data, size);
    zmd5_final(&ctx, result);
