...
typedef struct zfinder_redis_t zfinder_redis_t;
struct zfinder_redis_t {
    char *key;
    int fd;
    zstream_t *fp;
};
...
static int _close(zfinder_t * finder)
{
    ...
    my_db = (zfinder_redis_t *) (finder->db);
...
static int ___get(zfinder_t * finder, const char *query, zbuf_t * result, int timeout)
{
    ...
    for (i = 0; i < 2; i++) {
        ...
        if (buf[0] != '$') {
            zbuf_printf_1024(result, "zfinder_get: %s : read error, need $", finder->title);
            continue;
        }
        len = atoi(buf + 1);
        ...
        ret = ___zfinder_read_line(my_db->fp, buf, 102400, &rend);
        if (ret != 0) {
            zbuf_printf_1024(result, "zfinder_get: %s : read error, only need \\r\\n", finder->title);
            continue;
        }
        return 1;
    }
...
int zfinder_create_redis(zfinder_t *finder)
{
    ...
    my_db = (zfinder_redis_t *)zcalloc(1, sizeof(zfinder_redis_t));
    finder->db = my_db;
    finder->close = _close;
    finder->get = _get;
